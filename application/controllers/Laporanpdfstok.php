<?php
Class Laporanpdfstok extends CI_Controller{
...
$pdf->Image('assets/image/logopgn.png',0.8,1.0,4.8,1.5);
...
$pdf->ln(1);
$pdf->SetFont('Arial','B',12);
$pdf->Cell(18.5,0.7,"Laporan Data Stok",0,10,'C');
...
$pdf->Cell(3, 0.8, 'Lokasi', 1, 0, 'C');
$pdf->Cell(2.5, 0.8, 'Kondisi', 1, 0, 'C');
$pdf->Cell(2,0.8, 'Stok', 1, 0, 'C');

$pdf->SetFont('Arial','',8);


 $query =  $this->db->select('stok.*,barang.*,kondisi.*,lokasi.*');
...
$this->db->join('kondisi','kondisi.id_kondisi	= stok.kondisi');
 ...
 //        foreach ($data_matriks as $row){
    ...
    $pdf->Cell(6,0.8,$row->nama_barang,1,0, 'C');
    $pdf->Cell(3,0.8,$row->lokasi_barang,1,0, 'C');
    $pdf->Cell(2.5,0.8,$row->kondisi_barang,1,0, 'C');
    $pdf->Cell(2,0.8,$row->stok,1,0, 'C');
}
